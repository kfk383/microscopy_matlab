...
function clear_vars
global objects scale_box scale_line scale position filename bw img_file index
clear objects scale_box scale_line scale position filename bw img_file index;
...
global objects
objects(get_index) = [objects(get_index), obj];

function undo_object
global objects
if not(isempty(objects(get_index)))
    objects(get_index, end) = [];
end
...
axes(handles.axes1);
images = get_img_file;
imshow(string(images(get_index)));
...
objects_arr = get_objects;
for i = 1 : length(objects_arr)
    bw = get_bw;
    BW2 = bw(i);
